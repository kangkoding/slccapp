	...
	<div class="alert">
		<div class="media">
			<a class="pull-left" href="{{ base_url().permalink($p->permalink).$p->slug }}">
				...
				if ($p->featured_image != '') {
				?>
					<img class="media-object" src="{{ base_url('assets/images/').$p->featured_image }}" style="height:150px; width: 700px;">
				<?php } else { ?>
			...
			<br>
			<p><?php if ($lang != 2) {
					echo strip_tags(substr($p->isi, 0, 400));
				} else {
					echo substr($p->content, 0, 150);
				} ?><a href="{{ base_url().permalink($p->permalink).$p->slug }}"> Read More</a> </p>
