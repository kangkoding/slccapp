...
class Template_model extends CI_Model {
	...
	function news_section()
	{
		...
		//
			$r1 = $this->db->order_by('arrange','asc')->get('news_section',$limit,0)->result();
			$array1 = array();
			foreach ($r1 as $row) {
				$this->db->select('p.id,p.judul,p.created,p.slug');
				$this->db->from('post as p');
				...
				$array1[] = array('title'=>$row->title,'data'=>$dt);
			}
			$r2 = $this->db->order_by('arrange','desc')->get('news_section')->row();
				...
				$array2 = array('title'=>$r2->title,'data'=>$dt);
		//
		$array = array(
					'top'=>$array1,
					'bottom'=>$array2,
